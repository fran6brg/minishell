...
/*   Updated: 2020/02/10 19:22:05 by fberger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_new_str_with_quotes(char *s, char *quote, char **strs, int *str_i)
{
	...
	if (PARSE)
		printf("     ft_new_str_with_quotes | strs[%d] = %s | len = %d\n", *str_i, strs[*str_i], j - offset + 1); // pour debug
	*str_i += 1;
	return (1);
}
...
int	ft_new_str(char *s, int *i, char **strs, int *str_i)
{
	...
	j = *i;
    offset = 0;
	while (s[j] && !is_separator(s[j]) && !is_redirect(s[j]) && !is_quote(s[j]) && s[j] != '|')
	{
        if (s[j] == '\\' && (j == 0 || s[j - 1] != '\\'))
            offset++;
        j++;
